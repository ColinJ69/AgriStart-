...
from .weather import get_weather_soil_data
import requests
import torch
...
def recommend_grants(State, Area, Native):
    grants = pd.read_csv('https://github.com/ColinJ69/Farmers_Friend/raw/main/Data/Grants.csv')
    df = grants[(grants['State'] == State or grants['State'] == 'U.S.') & (grants['Area'] == Area) & (grants['Native'] == Native)]
    best_grants = df[['Name', 'Amount', 'Requirements']]
    return best_grants
